(function () {
    ...
    function renderCart() {
        ...
        Object.keys(cart).forEach(key => {
            ...
            // quantity controls for side cart: - [qty] +
            const qtyWrapper = document.createElement('div');
            qtyWrapper.className = 'btn-group btn-group-xs';
            qtyWrapper.style.margin = '0 8px';
            ...
            qtySpan.className = 'btn btn-light disabled';
        ...
        // If no items, show a sad face placeholder
        if (Object.keys(cart).length === 0) {
            const emptyLi = document.createElement('li');
            emptyLi.className = 'list-group-item text-center';
            emptyLi.innerHTML = '<span class="sad-face">:(</span> Váš košík je prázdny.';
        ...
        totalAmountEl.setAttribute('data-total', totals.total.toFixed(2)); // expose current total for other scripts (e.g. QR/payment helpers)
        if (totalDiscountEl) {
    ...
    function renderFavorites() {
        ...
        favIds.forEach(id => {
            ...
            priceSpan.innerHTML =
                '<span style="text-decoration:line-through;color:#999;margin-right:.4rem;">€' +
                price.toFixed(2) +
                '</span><span> €' + discountedPrice + '</span>';

            // + Add to cart (small)
            const addBtn = document.createElement('button');
            addBtn.type = 'button';
            addBtn.className = 'btn btn-xs btn-success btn-add-cart';
            addBtn.style.marginLeft = '6px';
            addBtn.textContent = '+';
            addBtn.title = 'Pridať do košíka';
            addBtn.addEventListener('click', function (ev) {
                ev.stopPropagation(); ev.preventDefault();
                addToCartById(id, img);
            });

            // - Reduce quantity in cart (small)
            const decBtn = document.createElement('button');
            decBtn.type = 'button';
            decBtn.className = 'btn btn-xs btn-default btn-dec-qty';
            decBtn.style.marginLeft = '2px';
            decBtn.textContent = '-';
            decBtn.title = 'Znížiť množstvo v košíku';
            decBtn.addEventListener('click', function (ev) {
                ev.stopPropagation(); ev.preventDefault();
                changeQuantity(id, -1);
            });

            // x Remove from favorites (small)
            const delBtn = document.createElement('button');
            delBtn.type = 'button';
            delBtn.className = 'btn btn-xs btn-danger btn-remove-fav';
            delBtn.style.marginLeft = '4px';
            delBtn.textContent = 'x';
            delBtn.title = 'Odstrániť z obľúbených';
            delBtn.addEventListener('click', function (ev) {
                ev.stopPropagation(); ev.preventDefault();
                toggleFavorite(id);
                renderFavorites();
